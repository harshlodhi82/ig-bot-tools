...
export class Utils {
    ...
    static randomWait(time: number): Promise<void> {
        return new Promise((resolve) => { setTimeout(() => { resolve() }, this.getRandomNumber(time, time + this.getRandomNumber(1000, 10000))) })
    }
    ...
    static arrayToBatches<T>(array: T[], batchSize: number): T[][] {
        const batches: T[][] = [];
    ...
    static differances<T>(arrayOne: T[], arrayTwo: T[]): T[] {
